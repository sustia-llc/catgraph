...
| Frobenius axioms verification | ✅ | tests/frobenius_axioms.rs (14 tests) | The nine equations — unitality and counitality split into left/right halves, so eleven — built on **both** sides and decided per carrier: `Cospan` and `Corel` by `CospanCanon` on the nose, `CospanAlgebraMorphism<PartitionAlgebra, _>` by `CospanCanon` after dropping bubbles (with the bubble counts themselves pinned), `FrobeniusMorphism` through `frobenius_to_cospan`. Riders for the zigzag identities and for the braiding being a real crossing, neither of which the nine can see. Alongside it, `tests/frobenius_laws.rs` (8 tests) — `identity_compose_identity`, `braiding_involution`, `spider_fusion`, `unit_counit_scalar`, `composition_associativity`, `monoidal_product_domains`, `from_decomposition_roundtrip`, `special_frobenius_morphism_consistency` — pins the *presentation*-level laws `FrobeniusMorphism`'s normalizer decides. ⚠ Not equation pins, and renamed at #283 to say so: the `src/hypergraph_category.rs` test mod (`unitality_left_arities`, `counitality_left_arities`, `associativity_arities`, `frobenius_law_lhs_arities`, `special_frobenius_arities`, `zigzag_via_trait_arities`, `frobenius_morphism_special_arities`) and the H_Part trio in the equivalence integration file assert **domains and codomains only** — `frobenius_law_lhs_arities` builds one side and no more — and stayed 19/19 green under a non-merging μ *and* a non-splitting δ |
