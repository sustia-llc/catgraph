...
> **Method:** re-read paper, cross-check each numbered item against source; spot-check all ✅/⚠️ location claims; run the full test suite green.
>
...
> `hypergraph_category`, `hypergraph_functor`, `equivalence`, `finset`, `monoidal`, `cospan_canon`) is fully intact.
>
> Coverage of `catgraph` (core) against F&S 2019. Anchors reconciled against the cached paper text + PDF (2026-07-13).
...
| Eq 5: hierarchy of category types (cat → mon → traced → hyper) | ⚠️ | — | implicit; no explicit `TracedMonoidalCategory` layer in core (hypergraph subsumes traced — the Eq 5 hierarchy's own point). Sibling `catgraph-applied::trace` provides a concrete partial trace over the Kronecker HC. |
| Eq 6: operadic substitution as a compositional view | ✅ | operadic.rs (trait) | concrete impl in `catgraph-applied::wiring_diagram`; the `Operadic` *trait* stays in catgraph core |
...
| Frobenius axioms verification | ✅ | tests/canonical.rs | The nine equations — unitality and counitality split into left/right halves, so eleven — built on **both** sides and decided for **four named carriers over three decision paths**: `Cospan` by `CospanCanon` on the nose; `Corel` by the *same* computation, since none of the eleven composites births a mid-composition bubble — the only thing `Corel::compose` does differently from the wrapped value since #351 (measured in `corel_equations_recompute_the_cospan_equations` — its independent content is the joint-surjectivity assertion inside that same test, not the equation row). ⚠ `Corel` is **not** a transparent newtype in general any more: #351 overrode `Composable::compose` to restrict the pushout to the outer boundary, and this row stayed green throughout, which is recorded in that test's docstring as the measured limit of what it pins; `CospanAlgebraMorphism<PartitionAlgebra, _>` by `CospanCanon` after dropping bubbles (with the bubble counts themselves pinned), the one path that shares nothing with `Cospan`; and `FrobeniusMorphism` through `frobenius_to_cospan`, which reads the battery's generators η, ε, μ, δ and id *as* the corresponding `Cospan` generator and so shares that half — σ excepted: `generator_to_cospan` builds the braiding as a hand-written literal, not through `Cospan::from_permutation_on_domain`, so the two rows take different paths there and the battery pins σ separately. ⚠ That map was briefly **two** public functions — `frobenius::` (#283) and `cospan_algebra::` (#284), landed on parallel branches — unified at #336 after `frobenius::to_cospan_pin` measured them equal up to `canonical_form` over 363 terms while both bodies were live (the space was widened to 383 in the same PR, once the retired algorithm had become the oracle; 212 distinct images there — 209 when the space was widened, moved to 212 at catgraph #350, which stopped cancelling `η;ε` so eleven random terms changed image — with a diversity floor beside the size assert, since agreement over 383 identities would be agreement about nothing); the `frobenius::` path is now a re-export and the retired algorithm survives there as the differential oracle. Riders for the zigzag identities and for the braiding being a real crossing, neither of which the nine can see, plus `spider_fusion_needs_a_wire_between_the_two_spiders` — #283 found the normalizer's Rule 4 fusing `Spider(z, m, 0) ; Spider(z, 0, k)`, wiring two disconnected components together, and added two redundant defenses — the `n >= 1` guard on Rule 4 and the zero-output filter on the `target_side_placement` lookup; the pin covers their conjunction (deleting either alone stays green, measured). ⚠ **Carrier limit**: four *named* carriers, not every implementor — `catgraph-applied`'s `PetriNet` and `DecoratedCospan` also implement `HypergraphCategory`; their Def 2.5 pins are `decorated_cospan_battery` and `petri_net_battery` in applied's `tests/hypergraph_laws.rs`. Alongside it, `tests/frobenius_laws.rs` — `identity_compose_identity`, `braiding_involution`, `spider_fusion`, `unit_counit_scalar`, `composition_associativity`, `monoidal_product_domains`, `from_decomposition_roundtrip`, `special_frobenius_morphism_consistency` — pins the *presentation*-level laws `FrobeniusMorphism`'s normalizer decides. ⚠ Not equation pins, and renamed at #283 to say so: the `src/hypergraph_category.rs` test mod (`unitality_left_arities`, `counitality_left_arities`, `associativity_arities`, `frobenius_law_lhs_arities`, `special_frobenius_arities`, `zigzag_via_trait_arities`, `frobenius_morphism_special_arities`) and the H_Part trio in the equivalence integration file assert **domains and codomains only** — `frobenius_law_lhs_arities` builds one side and no more. Each needs its own mutant, because they touch different code: the src mod stayed green under a non-merging μ *and* a non-splitting δ on `Cospan`'s `HypergraphCategory` impl (19/19 measured at #283, `hypergraph_category.rs:227`; the new battery catches both on three carriers), while the H_Part trio never reaches that impl — it builds from `PartMorph::multiplication_in`/`comultiplication_in` — so its mutant is `multiplication_in`'s right leg `[0, 0, 0]` → `[0, 1, 0]`: the trio stays green, and `def_2_5_equations_on_every_carrier`, `cospan_algebra_morphism_bubble_ledger` and the zigzag rider go red |
...
| Ex 2.16: FdVect with chosen basis as hypergraph cat | ✅ | sibling `catgraph-applied::mat_kron` | Kronecker/Hadamard genuine HC over a rig; Hadamard SCFM μ/δ/η/ε as inherent generators, speciality δ;μ=id (n=2,3,5). Realized in the applied crate, not core. |
...
**Reconciliation note:** catgraph's concrete operadic machinery — `wiring_diagram.rs`, `e1_operad.rs`, `e2_operad.rs`, `temperley_lieb.rs`, `linear_combination.rs`, `petri_net.rs` — is **not** part of the F&S 2019 paper's numbered content, and now lives in `catgraph-applied` (workspace sibling). The core `Operadic` trait (`operadic.rs`) stays in catgraph as the abstract interface. See "Reconciliation with catgraph-applied" section below.
...
| Eq 13: zigzag identities | ✅ | tests/compact_closed.rs | content-pinned against `Cospan` canonical forms since [#284](https://github.com/sustia-llc/catgraph/issues/284) — before that the suite asserted interfaces only |
...
| Prop 3.8: Cospan ≃ theory of special commutative Frobenius monoids | ✅ | cospan_algebra.rs::cospan_to_frobenius + hypergraph_functor.rs::CospanToFrobeniusFunctor + cospan_canon.rs::CospanCanon | post-audit: CospanCanon / Cospan::canonical_form (#100, read surface #254, from_parts/to_cospan round-trip #261) give the decidable cospan-equality-up-to-apex-iso that Cospan_Λ's morphisms (iso classes) are stated over; the functor's image is pinned at the content level since #285 (the identity fast path had claimed the all-merged m→m cospan was the identity) |
...
| Prop 4.6: Part_Λ is initial cospan-algebra | ✅ | tests/cospan_algebra.rs::prop_4_6_* | proptest verifies unit preservation, naturality, and monoidal coherence of the unique map α_x(c) = A.map_cospan(c, A.unit()) for PartitionAlgebra → PartitionAlgebra and PartitionAlgebra → NameAlgebra; since #285 the Part-side proptests also compare both legs over a mixed-label generator (two apex classes share a label, so the leg carries information the codomain does not) and the naturality square uses a non-identity downstream cospan |
...
| Eq 33: six required cospans (id, braiding, μ, η, δ, ε) as ∅ → X⊕Y morphisms | ✅ | equivalence.rs identity_in/multiplication_in/comultiplication_in/unit_in/counit_in + from_permutation_on_domain/from_permutation_on_codomain | braiding constructors renamed and FIXED at v0.15.0 (#258): the pre-#258 CospanAlgebraMorphism "braiding" was not one (identity permutation ↛ identity morphism); now pinned in catgraph-applied/tests/braiding_cross_carrier.rs |
| Lemma 4.9: F_α io functor from morphism α: A→B | ✅ | equivalence.rs::functor_from_algebra_morphism | free function lifting a monoidal natural transformation α: A → B to F_α: H_A → H_B by pointwise application; tests in tests/equivalence.rs verify identity and composition preservation for both F_id and the non-trivial α = cospan_to_frobenius case; since #285 the witnesses are non-identity morphisms (μ, δ, μ;δ) and the F_id case compares the transported element leg by leg. Note Part is initial, so its only self-morphism *is* the identity — the F_id case pins functor_from_algebra_morphism's transport, not functoriality of a non-trivial α |
...
| Item | Status | catgraph location | Notes |
|---|---|---|---|
| Thm 4.13: Hyp_OF(Λ) ≃ Lax(Cospan_Λ, Set) | ✅ | equivalence.rs + tests/equivalence.rs | roundtrip verified for PartitionAlgebra |
...
*(Ex 2.16 FdVect-with-chosen-basis is no longer missing — realized in sibling `catgraph-applied::mat_kron`.)*
...
| different-target-category | 10 (§2.2 LinRel, §2.3 Hyp 2-cat + Ex 2.19, §2.4 LinRel, §3.4 strictification, §4.3 LinRel) | Stay missing in catgraph core. Would require LinRel / 2-categorical machinery — a different crate's job. (Ex 2.16 FdVect was here; now realized in sibling `catgraph-applied::mat_kron`.) |
...
**catgraph implements Theorem 1.2 in its per-Λ form (which is Thm 4.13)**, with full bidirectional functoriality (Lemmas 4.3 and 4.9), all six structural cospans of §4.2, and Props 3.1–3.4 on compact closed structure, using `PartitionAlgebra` and `NameAlgebra` as worked examples.
...
## Reconciliation with catgraph-applied

catgraph core tracks the Fong-Spivak 2019 *Hypergraph Categories* paper strictly. The complementary applied-CT track lives in the workspace sibling `catgraph-applied`, anchored to Fong & Spivak 2018 (*Seven Sketches in Compositionality*); its section-by-section audit is [`catgraph-applied/docs/FS18-AUDIT.md`](../../catgraph-applied/docs/FS18-AUDIT.md).

### Modules that are **not** part of F&S 2019 numbered content

These live in `catgraph-applied`. They do not appear in the audit tables above because the 2019 paper does not cover them. The audit's coverage percentages are unaffected by the move.

| Module | Nature | Paper relationship | Home crate |
|---|---|---|---|
