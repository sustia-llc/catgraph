...
| Frobenius axioms verification | ✅ | tests/frobenius_laws.rs | 8 tests: `identity_compose_identity`, `braiding_involution`, `spider_fusion`, `unit_counit_scalar`, `composition_associativity`, `monoidal_product_domains`, `from_decomposition_roundtrip`, `special_frobenius_morphism_consistency`; the literal Def 2.5 equation pins (unitality, counitality, Frobenius law, speciality) are in src/hypergraph_category.rs's test mod (`unitality_left`, `counitality_left`, `frobenius_law`, `special_frobenius`, `frobenius_morphism_special`) |
