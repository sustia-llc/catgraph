...
| `cospan_algebra.rs` | `CospanAlgebra`, `PartitionAlgebra`, `NameAlgebra`, `functor_induced_algebra_map`, `cospan_to_frobenius`, `frobenius_to_cospan` | Lax monoidal functors Cospan → Set (§2.1), Lemma 4.3 natural transformation (io; see Feature Map), and the two translations between `Cospan_Λ` and `FrobeniusMorphism_Λ` |
...
| Lemma 3.6, Prop 3.8 | `cospan_algebra.rs`, `hypergraph_functor.rs` | `cospan_to_frobenius` + `CospanToFrobeniusFunctor` — epi-mono decomposition into Frobenius generators. `frobenius_to_cospan` goes the other way — the cospan a string diagram denotes: layers tensored blockwise, composed by pushout. One function, defined in `cospan_algebra.rs` and re-exported as `frobenius::frobenius_to_cospan` (it was briefly two, unified at #336). Prop 3.8's correspondence is between SCFMs in `C` and strict symmetric monoidal functors *out of* `Cospan`, and applying it to the SCFM structure `Cospan_Λ` carries (Ex 2.8) is what licenses interpreting a Frobenius term as a cospan. Composing with `Cospan::canonical_form` gives a semantic equality on terms — the decision procedure for Def 2.5 equality of two parallel `FrobeniusMorphism`s, whose own `PartialEq` compares presentations. It used to be **incomparable** with SCFM-equality on scalars, neither sound nor complete in both measured directions, because the layer simplifier quotiented by the extra-special axiom `ε ∘ η = id_I` while `Cospan` is the merely *special* theory. That rule was deleted at #350 — both ends are the special theory now, both refuting witnesses are pinned the other way up, and scalars survive both directions; two closed counterexamples are still not a proof of soundness or completeness. See the #350 entry of the `catgraph` CHANGELOG at tag [`v0.23.0`](https://github.com/sustia-llc/catgraph/blob/v0.23.0/catgraph/CHANGELOG.md) (and, for the history, "Known discrepancy — scalars"). |
