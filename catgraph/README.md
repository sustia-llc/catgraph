...
| `cospan_algebra.rs` | `CospanAlgebra`, `PartitionAlgebra`, `NameAlgebra`, `functor_induced_algebra_map`, `cospan_to_frobenius`, `frobenius_to_cospan` | Lax monoidal functors Cospan → Set (§2.1), Lemma 4.3 natural transformation (io; see Feature Map), both directions of Prop 3.8 |
...
| Lemma 3.6, Prop 3.8 | `cospan_algebra.rs`, `hypergraph_functor.rs` | `cospan_to_frobenius` + `CospanToFrobeniusFunctor` — epi-mono decomposition into Frobenius generators. `frobenius_to_cospan` is the semantics direction, and (since Cospan is the theory of special commutative Frobenius monoids) composing it with `Cospan::canonical_form` decides equality of Frobenius terms. Scalars are the one thing neither direction preserves — see the `catgraph` CHANGELOG's "Known discrepancy — scalars". |
