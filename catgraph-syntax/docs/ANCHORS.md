...
| `arrow_seam` (owned Arrow algebra) | — | #12 single-seam precedent (catgraph-dl `src/endofunctor.rs`); crate-owned definitions since #222 (re-exports of `deep_causality_haft` before that); {`Arrow`,`Compose`,`Split`,`Id`,`Lift`} consumed by S5 `Traced`; {`arrow`,`ArrowBuilder`,`First`,`Second`,`Fanout`} reserved surface | live (S1 as re-export, owned at #222); {consumed, reserved} split (S5) |
...
| `frobenius::FrobeniusOr` | Hypergraph Categories | Def 2.5 (the SCFM generators μ/η/δ/ε as a sum over `G`); Def 2.12 (a hypergraph category equips **each object** with a Frobenius structure — hence the `Λ` payload on the four spider variants) | live (S4); Λ-colored since #79 P3a, colour-annotated tokens (`mu@A`) since P3b |
...
| `frobenius::scfm_equations` | Hypergraph Categories | Def 2.5 (the **nine** equations, per Ex 2.8's count), at one colour | live (S4); per-colour since #79 P3a |
