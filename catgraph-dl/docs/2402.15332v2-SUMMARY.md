...
> **Paper:** Gavranović\*, Lessard\*, Dudzik\*, von Glehn, Araújo, Veličković, *Position: Categorical Deep Learning is an Algebraic Theory of All Architectures* ([arXiv:2402.15332v2](https://arxiv.org/abs/2402.15332), 6 Jun 2024; ICML 2024 PMLR 235:15209-15241).
>
> **Title parsing reminder:** the paper's title carries the ICML `Position:` prefix; read the rest as "Categorical Deep Learning is an Algebraic {Theory of All Architectures}", *not* "Categorical Deep Learning is an {Algebraic Theory} of All Architectures".
>
...
| Citation, theorem statements, formal definitions, Haskell types, architecture diagrams | **Part II** (faithful paper rendering of §1–§4 and appendices A, B, C, H, I, J — the paper's appendices D, E, F, G are not rendered — plus an editorial consolidation of the inline Haskell types) |
...
| Implementation guidance (Haskell types, RNN-as-coalgebra, Para 2-category) | Part II Appendices I, J |
| The matrix-convention trap in the worked examples | Part II Appendix H.1 / H.3 (inline ⚠️ CAREFUL boxes) |
...
| **Free monads (Proposition B.18, Appendix B.2)** | Explicit `FreeMnd(F) = Fix(X ↦ F(X) + Z)` | Absent |
...
- **Free / cofree (co)monad** explicit formula `Fix(X ↦ F(X) + Z)` (Proposition B.18, Appendix B.2) — the construction that turns endofunctor algebras into monad algebras and *unrolls* RNN cells
- **Worked equivariance/invariance derivations** for $\mathbb{R}^{\mathbb{Z}_2}$ (Appendix H.1, H.3 — both subject to the display-vs-arithmetic matrix-convention mismatch flagged in the merged doc)
...
| Implementation guidance (Haskell types, RNN-as-coalgebra) | Paper appendices I, J |
...
> **Note on title:** Read the post-prefix title as "Categorical Deep Learning is an Algebraic {Theory of All Architectures}", *not* "Categorical Deep Learning is an {Algebraic Theory} of All Architectures".
...
**Generalised definition** (Definition G.1 — from a monoidal category $(\mathcal{M}, \otimes, I)$ and an $\mathcal{M}$-actegory $\mathcal{C}$, define $\text{Para}_{\blacktriangleright}(\mathcal{C})$):
...
> **Editorial:** §4 is unnamed continuous prose in the paper. The four headings below are this document's, not the paper's — cite them as "§4 *New Horizons*", never as titled subsections.
...
##### Worked Invariance Example (paper Ex H.3, eq. (12); rendered from summary Doc 2, §7.7)

For **invariance** (codomain has trivial action) with the same $\mathbb{Z}_2$ swap setup, the paper's eq. (12) gives:
...
> **⚠️ CAREFUL — the invariance derivation is convention-dependent.** Working from $W = \begin{bmatrix} w_1 & w_3 \\ w_2 & w_4 \end{bmatrix}$ and requiring $f(x_1, x_2) = f(x_2, x_1)$ component-wise:
> - $w_1 x_1 + w_3 x_2 = w_1 x_2 + w_3 x_1 \;\Rightarrow\; w_1 = w_3$
> - $w_2 x_1 + w_4 x_2 = w_2 x_2 + w_4 x_1 \;\Rightarrow\; w_2 = w_4$
>
> Under that column-major reading the invariance constraint is $w_1 = w_3 \text{ and } w_2 = w_4$, yielding $W = \begin{bmatrix} w_1 & w_1 \\ w_2 & w_2 \end{bmatrix}$ (**columns** identical). The paper's eq. (12) gives "$w_1 = w_2 \text{ and } w_3 = w_4$" with $\begin{bmatrix} w_1 & w_3 \\ w_1 & w_3 \end{bmatrix}$ (**rows** identical). **Rederive from scratch before citing.**
>
...
**Example H.8 (Unfolds to Moore machines are terminal $(O \times -)$-coalgebras [sic]).** Consider the endofunctor $O \times (I \to -)$ from Example H.7, and any other $(O \times -)$-coalgebra [sic] $(X, \langle o, n \rangle)$ to $(\text{Moore}_{O,I}, \langle \text{output}, \text{next} \rangle)$. Then the map $f_{o,n}$ is necessarily an *unfold* to a Moore machine, a corecursive function:

```haskell
f_m :: x -> Moore o i
f_m x = MkMoore (m_1 x) (\i -> f_m (m_2 x i))
```
...
> **[sic] — Ex H.8 endofunctor label.** The paper's title and body print "$(O \times -)$-coalgebras" for Moore machines, whose endofunctor is $O \times (I \to -)$; as printed, the title also contradicts Remark H.6, which makes *streams* the terminal $(O \times -)$-coalgebra. Rendered here as the paper prints it rather than normalised; see the AUDIT's paper-fidelity caveats.
...
*(the paper's listing prints `cell_n x`, dropping the parameter that its own prose supplies — "whose tail is the stream produced by $f_{o,n}$ at $\text{cell}_n(p, x)$"; reproduced as printed. The consolidated editorial listing below writes `cell_n p x`.)*
...
*(the paper prints this line as `cell^rcsv p inr (f (f_rcsv p l) (f_rcsv p r)` — a stray `f` and unbalanced parentheses; the balanced reading is shown.)*
...
**Example J.4 (Iterated Mealy machine cell).** Consider a coalgebra $(P, \text{cell}^{\text{Mealy}})$ of $\mathbf{Para}(I \to O \times -)$: *(the paper's text says "algebra" here — a wording slip contradicting Ex 2.11/I.4, diagram (20), and J.2/J.5; corrected to coalgebra, see the AUDIT's paper-fidelity caveats)*
...
> **⚠️ CAREFUL — global flag for matrix worked-examples (§H.1, §H.3 worked invariance).** The paper *displays* $W$ column-major but does the eq. (11)/(12) arithmetic row-major, so the concrete weight-matrix derivations for the $\mathbb{Z}_2$ swap action are correct only under a single consistent reading of that convention. The structural conclusions — *equivariance ⇒ circulant/symmetric matrix ⇒ CNN*; *invariance ⇒ output is a symmetric function of inputs* — are correct and standard in signal processing, but **the intermediate row-by-row substitutions in both documents should be rederived from your own conventions before being cited or reproduced in lecture material.**
