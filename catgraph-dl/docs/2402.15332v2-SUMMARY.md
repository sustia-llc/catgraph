...
>
> **Status (pre-reboot release process, "release rule 7" — a numbered release-rule list that predates this repo's CLAUDE.md).** This SUMMARY.md is the locked primer for the agent paper-audit reviewer briefed against the catgraph-dl source tree. The CDL paper anchor is the arXiv PDF ([arXiv:2402.15332v2](https://arxiv.org/abs/2402.15332), 539 KB — well below the ~10 MB hybrid paper-audit refinement threshold), fetched at review time, so the reviewer also has direct cold-read access to the PDF. This primer pre-encodes (a) the published-paper section coverage, (b) the transcript-vs-paper provenance distinction, and (c) two ⚠️ CAREFUL caveats on Appendix H worked-example arithmetic that the reviewer should not re-derive from scratch.
>
> **Companion artefacts:**
> - [arXiv:2402.15332v2](https://arxiv.org/abs/2402.15332) — the paper itself (fetch the PDF from arXiv; PDFs are not kept in-tree).
...
>
> **Companion artefacts in the workspace `.claude/` umbrella** (private working artifacts; `.claude/` is gitignored and does not ship in this repo):
