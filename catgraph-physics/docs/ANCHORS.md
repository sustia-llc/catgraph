...
- **[Wil74]** K. G. Wilson, *Confinement of quarks*, Phys. Rev. D **10**, 2445
  (1974) — the lattice-gauge source for matrix link variables, path-ordered
  holonomies, the Wilson loop and gauge transformation by vertex conjugation.

## Provenance table

| Site | Content | Source | Status |
|---|---|---|---|
| `multiway/evolution_graph.rs` module header | irreducibility ⟺ the computation→cobordism map is a **pure** symmetric monoidal functor; reducibility = deformation away from exactness | [Gor23] | ✅ cache-verified (audit Phase 4, PR #125 — an earlier inverted gloss was fixed there) |
...
| `multiway/branchial_analysis.rs` (`multiway_betweenness`, `multiway_katz`, `MultiwayEvolutionGraph::to_petgraph`) | betweenness (Brandes) and Katz centrality on multiway evolution graphs; eigenvector centrality deliberately NOT shipped (nilpotent adjacency of a step-graded DAG) | **catgraph extrapolation** — standard-literature algorithms via rustworkx-core; no Gorard content; rationale in the CHANGELOG's v0.14.0 section at tag `v0.23.0` | added v0.14.0 (#161, #162) |
| `hypergraph/gauge.rs` (`GaugeGroup`, `HypergraphRewriteGroup`, `LinkVariable`, `DMatrix<f64>` / `Rotation3<f64>` / `UnitQuaternion<f64>` / `Isometry3<f64>` link variables, path-ordered loop holonomy, Wilson loop as normalized trace, gauge transformation by vertex conjugation, plaquette action, "causal invariance ⟺ flat gauge field / identity holonomy") | gauge-theoretic reading of hypergraph rewriting | inspired by [Gor20a]'s causal-invariance-as-gauge-covariance; the matrix link variable, the `Rotation3<f64>` and `UnitQuaternion<f64>` SO(3) link variables, the path-ordered holonomy `U_k · … · U_1`, the Wilson loop as `tr(H) / n` and the gauge transformation `U_{x→y} ↦ g_y · U · g_x⁻¹` are standard lattice-gauge-theory machinery — [Wil74] (†); the hypergraph-lattice keying of the links, the `Isometry3<f64>` link variable read as `tr(H) / 4` over its 4 × 4 homogeneous matrix, and the "causal invariance ⟺ flat gauge field (identity holonomy)" equivalence are a **catgraph interpretive gloss**, not a stated theorem of any cached paper | [Gor20a] inspiration cache-verified (abstract + §2.3, 2026-08-17); [Wil74] (†); the ⟺-gloss remains a catgraph interpretation |
