...
| `hypergraph/gauge.rs` (`GaugeGroup`, `HypergraphRewriteGroup`, `DMatrix<f64>` link variables, path-ordered loop holonomy, Wilson loop as normalized trace, gauge transformation by vertex conjugation, plaquette action, "causal invariance ⟺ flat gauge field / identity holonomy") | gauge-theoretic reading of hypergraph rewriting | inspired by [Gor20a]'s causal-invariance-as-gauge-covariance; the matrix link variable, the path-ordered holonomy `U_k · … · U_1`, the Wilson loop as `tr(H) / n` and the gauge transformation `U_{x→y} ↦ g_y · U · g_x⁻¹` are standard lattice-gauge-theory machinery — [Wil74] (†); the hypergraph-lattice keying of the links and the "causal invariance ⟺ flat gauge field (identity holonomy)" equivalence are a **catgraph interpretive gloss**, not a stated theorem of any cached paper | [Gor20a] inspiration cache-verified (abstract + §2.3, 2026-08-17); [Wil74] (†); the ⟺-gloss remains a catgraph interpretation |
