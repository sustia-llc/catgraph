...
| `prop::presentation::kb` | Congruence-closure decision procedure (DST 1980 signature-table variant) — the default `eq_mod` backend, with an atom-canonical `smc_refine` fixpoint (BoolRig d=2 collisions 2574 → 1433 → 1301 post-#14 → 1142 post-E_18 → 972 post-#55 PR1 → 979 post-#55 PR2 → 980 post-#174 — the upticks are equational-reach churn, not NF regressions; canonicality is gated by the `smc_canonicality_probes`) |
