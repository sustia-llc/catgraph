...
| `mat_to_sfg` | `mat_to_sfg` realization — converse of Thm 5.53 via the Prop 5.56 / Ex 5.59 four-layer composite; round-trip `S(mat_to_sfg(M)) == M` tested (F&S Prop 5.56) |
| `sfg_to_colored` | `sfg_to_colored_expr` — a `SignalFlowGraph<R>` as the `ColoredExpr<SfgGenerator<R>>` the rewrite engine's entry points take: the underlying expression carried across unchanged, paired with the `mono_word` over its domain arity. A representation change, not a functor; crate extension, no paper anchor |
...
| `mat_f64` (feature `f64-rig`) | nalgebra bridge for `MatR<F64Rig>`: determinant, inverse, `solve` (`a ; x = b` by LU), SVD rank, 1-norm (largest absolute column sum), Frobenius norm, `DMatrix` roundtrip |
| `prop::presentation::kb` | Congruence-closure decision procedure (DST 1980 signature-table variant) — the default `eq_mod` backend, with an atom-canonical `smc_refine` fixpoint (BoolRig d=2 collisions 2574 → 1433 → 1301 post-#14 → 1142 post-E_18 → 972 post-#55 PR1 → 979 post-#55 PR2 → 980 post-#174 → 952 post-#57-a1 → 748 post-#189 — the upticks are equational-reach churn, not NF regressions; the #57-a1 drop is the SMC layer moving to content equality; and the #189 drop is the metric itself, whose bucket partition became connected components rather than a greedy class scan; canonicality is gated by the `smc_canonicality_probes`) |
...
- **Thm 5.60 test naming.** The module doc of `tests/graphical_linalg.rs` says
  what its `cc_completeness_tracking_*` tests measure (CC incompleteness vs the
  matrix ground truth, not Thm 5.60). They stay `#[ignore]`'d as a diagnostic.
...
See [`docs/FS18-AUDIT.md`](docs/FS18-AUDIT.md) for the section-by-section Seven Sketches coverage audit (Chapters 4–6, 63 items tracked; 85% of implementable items DONE, §5.4 Thm 5.60 closed via Functorial engine). Cross-linked from [`../catgraph/docs/FS19-AUDIT.md`](../catgraph/docs/FS19-AUDIT.md) "Reconciliation" section.
