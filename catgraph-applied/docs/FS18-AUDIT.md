...
> **Anchors:** reconciled against the cached paper text + PDF, 2026-07-13.
...
>
> Coverage of `catgraph-applied` against F&S 2018 (Chapters 4–6). Anchors reconciled against the cached paper text + PDF (2026-07-13).
...
| Def 5.50: Mat(R) prop of R-matrices | ✅ | catgraph-applied::mat | `MatR<R>` pure-rig matrix prop. F&S convention: morphism m→n is m×n matrix. Composable/Monoidal/SymmetricMonoidalMorphism over any `Rig`; block_diagonal tensor; rig-generic `trace` (diagonal sum, `None` off the square). `mat_f64` nalgebra bridge behind opt-in `f64-rig` feature. |
| Thm 5.53: prop functor S: SFG_R → Mat(R) | ✅ | catgraph-applied::sfg_to_mat | `sfg_to_mat` structural recursion over `PropExpr<SfgGenerator<R>>`; generator table matches Eq 5.52 exactly. Functoriality (S(f∘g) = S(f)·S(g), S(f⊗g) = S(f)⊕S(g)) and agreement with a basis-vector evaluator verified in `tests/canonical.rs`. |
...
| Prop 5.56: every matrix M∈Mat(R) is realized by some SFG (`S` is surjective/full) | ✅ | catgraph-applied::mat_to_sfg | `mat_to_sfg(M)` realizes the converse of Thm 5.53 via the Prop 5.56 / Exercise 5.59 four-layer composite (copy/discard ; scalars ; swaps/identities ; add/zero): exactly one path from input `i` to output `j` carrying the single scalar icon `M(i, j)`. The characteristic property `S(mat_to_sfg(M)) == M` is verified in `tests/canonical.rs` — the paper's Eq 5.57 2×2 template + Exercise 5.58's three matrices + empty-dimension edge cases, plus a round-trip proptest over all four rigs (BoolRig / Tropical / UnitInterval / F64Rig). Dual helpers `add_n`/`zero_n` added alongside `copy_n`/`discard_n`. |
| Eq 5.52: generator → matrix table (copy, discard, add, zero, scalar) | ✅ | catgraph-applied::sfg_to_mat + tests/canonical.rs | All 5 generator matrices verified in `tests/canonical.rs`. |

### §5.4 Graphical linear algebra (pp. 168–178)

| Item | Status | Location | Notes |
|---|---|---|---|
| Thm 5.60: presentation of Mat(R) from Frobenius + rig equations | ✅ | catgraph-applied::graphical_linalg + prop::presentation::functorial | **DONE** via the semantic Functorial engine. `matr_presentation<R>` builds all 18 equations from F&S p.170 / BE15 Theorem 2 relations (1)–(18) (Groups A cocomonoid, B monoid, C bialgebra, D scalar — D1/D3/D4/D5/D6/D7 instantiated for `rig_samples`, D2/D8 emitted once; D7 = scalar addition BE15 (12), D8 = zero scalar BE15 (14)). Decision procedure: `Presentation::eq_mod_functorial(a, b, &MatrixNFFunctor::<R>::new())` — complete by F&S Thm 5.53 + Thm 5.60 (proof via Baez-Erbele 2015 for fields, Wadsley–Woods arXiv:1505.00048 for commutative rigs, cf. BE15 §6; the isomorphism `Free(Σ_SFG)/⟨E_{18}⟩ ≅ Mat(R)` is realised by `sfg_to_mat`). **Syntactic (CC) decision remains incomplete by design** — Option A (atom-canonical `smc_refine` in `kb::CongruenceClosure`) plus the post-#14 layer-ordering NF plus the E_18 completion plus the #55-PR1 η-before-ε within-layer reorder plus the #55-PR2 component-anchored η sift plus the #174 column pass + free-site retirement reduce BoolRig d=2 collisions 2574 → 1433 → 1301 → 1142 → 972 → 979 → 980 → 952 → 748 but cannot reach zero (the PR2 and #174 upticks are equational-reach churn, not NF regressions; the #57-a1 drop is the SMC layer moving to content equality; and the final #189 drop is the metric itself changing — the bucket partition became connected components rather than a greedy class scan — canonicality is witnessed by the `smc_canonicality_probes`; see §15); the terminal Mat(R) decision path is the Functorial engine (resolved: functorial-terminal, #15; KB feasibility spike #57). The module doc of `tests/graphical_linalg.rs` says what its `cc_completeness_tracking_*` tests measure (CC incompleteness vs the matrix ground truth, not Thm 5.60 verification). They stay `#[ignore]`'d as a diagnostic, not a release gate. |
...
| Item | Status | Location | Notes |
|---|---|---|---|
| Def 6.52: Frobenius structure (μ, η, δ, ε + 9 axioms) | 🔗 | catgraph::frobenius | `FrobeniusOperation`; the equations are decided in core `tests/canonical.rs` |
| Def 6.54: spider s_{m,n} | 🔗 | catgraph::frobenius | `from_decomposition` constructs spiders from generators |
| Thm 6.55: spider theorem (connected diagrams = spiders) | 🔗 ✅ | catgraph::frobenius + `tests/spider_theorem.rs` | Explicit tests in catgraph core — `tests/spider_theorem.rs`, in two halves. **§1, five term-level tests** (#110) assert *structural* equality against `special_frobenius_morphism(m, n, z)` at (2,2), (3,1), (1,3), (0,0), (2,1) — these pin the **builder's own shape**, not the theorem: `FrobeniusMorphism`'s `Eq` is syntactic up to `two_layer_simplify`, so a connected diagram the builder did not itself build is structurally ≠ the spider (six such measured, #288). **§2 asserts the theorem itself** (#288), over a finite generated corpus rather than in general: 2113 terms built from η/ε/μ/δ/σ/id alone are checked at the *semantics*, via `frobenius_to_cospan` + `Cospan::canonical_form` — 1307 connected terms (both labels × (m,n) in 0..=4 × 0..=4 × four decoration variants, plus two scripted wide-waist families and random walks; depth to 21; 1109 carrying a σ) each land on one apex vertex covering all of 0..m / 0..n, equal to the spider's own canonical form. ⚠ `frobenius_to_cospan` is **not** a direction of Prop 3.8 — both of those are functors *out of* `Cospan` — and the row below is: Prop 3.8 licenses this map's *construction* from `Cospan_Λ`'s per-object SCFM structure (Ex 2.8), and `cospan_algebra`'s rustdoc *used to* record it as neither sound nor complete against SCFM on scalars, which is why the scalar-shaped exclusions below were drawn where they are. That gap closed at catgraph #350 (`two_layer_simplify`'s rule 3, the extra-special axiom, deleted; both witnesses now pinned the other way up) — two refuted counterexamples are still not a proof of soundness or completeness, but the **first two** exclusions (`m = n = 0` and component-closing recipes) had outlived their cause and were lifted at catgraph #353: both arms now assert `scalar_count()` against the recipe's own count of components touching neither boundary, each arm floors how many such terms it ranges over, and the connected arm reads its wire label off the recipe rather than off the boundary a `0 → 0` term does not have. The one remaining exclusion (component-free empty recipes) rests on a reason #350 does not touch — on the empty term the count assertion reads `0 == 0` — and #353 put it explicitly out of scope. The corpus's structural spread is disclosed in the test's own docstring and is not uniform: a one-wire *internal* layer boundary (the *interior waist*, which deliberately does not count the diagram's own boundary) splits a diagram into two strictly smaller connected pieces, on which the conclusion follows by induction. ⚠ That metric is evidence, not proof, in both directions, and `Built::is_wide_waist` says so: a cut of a string diagram is an antichain and need not be a boundary between two recipe layers, and at `m = n = 1` the phrase "factors as s_{m,1} ; … ; s_{1,n}" is satisfied trivially by `id ; D ; id` without splitting anything — **15** of the 1030 wide terms sit at that arity and are counted wide on that reading — that total is census-pinned; its split into 8 scripted and 7 random walks is a measurement, not a pin, so a scripted term leaving as a random walk arrived would keep the census green — while the 992 swept terms all have m, n ≥ 2 and do not rest on it. Before the wide families only 22 of 272 connected terms had interior waist ≥ 2, so `wide_waist_family` (16 shapes) and `wide_waist_permutation_family` (1488 terms — every permutation of the 2m middle wires of a δ-fan folded into a μ-fan) were added deliberately; the connected histogram is now {None: 45, 1: 232, 2: 23, 3: 619, 4: 388}, all three buckets pinned by the census with a floor on the wide one. Connectivity is decided by a disjoint-set over the *recipe*, never read off the oracle, and the verdict is used both ways: 759 disconnected recipes must denote exactly their own component count, and their 43 component-closing members exactly their own closed-component count in scalars. Recipes with no component at all (the empty term, 47 of them, on which the count assertion would read `0 == 0`) are the one shape excluded by design |
...
| Thm 6.77: Cospan_F is a hypergraph category | ✅ | catgraph-applied::decorated_cospan + petri_net | `impl HypergraphCategory<Lambda> for DecoratedCospan<Lambda, D>` realizes the theorem generically (any `D: Decoration`). `impl HypergraphCategory<Lambda> for PetriNet<Lambda>` specializes via `from_cospan`. Both carriers are decided in `tests/hypergraph_laws.rs`: Def 2.5's nine equations with unitality and counitality split into halves, the two snakes, and a Def 2.12 generator table the six `HypergraphCategory` implementors are each compared against. `PetriNet`'s key is the underlying cospan up to apex isomorphism — its generators each carry one transition and composition concatenates the lists, so the transition counts an equation's two sides hold are recorded separately, per side, in `petri_net_battery_transition_ledger`. `PetriNet` also participates in `tests/braiding_cross_carrier.rs` on both constructors and on `permute_side` (#272, reading 1: the declared boundary survives). |
...
1. **Thm 6.55 (spider theorem)** — ✅ **CLOSED in catgraph core**, at the semantics rather than at the term level. `tests/spider_theorem.rs` interprets a generated corpus of connected Frobenius diagrams through `frobenius_to_cospan` — the interpreting functor Prop 3.8 licenses out of `Cospan_Λ`'s SCFM structure (Ex 2.8), *not* one of Prop 3.8's own two directions, which both point the other way — and asserts that each canonicalises to a single apex vertex carrying every domain and codomain index, the spider's own canonical form, which `special_frobenius_morphism(m, n, z)` is then compared *to* (#288). It is a wide finite sample at one instantiation (`char`/`String`), not a proof, and the shape of the sample — including the interior-waist spread and the two wide-waist families (`wide_waist_family`, and the permutation sweep `wide_waist_permutation_family`) added to answer it — is stated in the test's own docstring. Its five older term-level tests (#110) assert *structural* equality against that builder at five arities; structural equality is not the Frobenius quotient, so those pin the builder's shape and not the theorem. Connectivity is decided by a disjoint-set over the construction and checked in both directions; component-free empty recipes are the one shape excluded by design. ⚠ `m = n = 0` and component-closing recipes were excluded too, on the special vs extra-special line, which **was decided at catgraph #350** (`two_layer_simplify`'s rule 3 deleted; the carrier is the special theory and a closed bubble now survives on both sides). Both were lifted at catgraph #353, which re-pointed each arm's scalar assertion at the recipe's own count of components touching neither boundary — see the Thm 6.55 row above.
...
The `MatrixNFFunctor` semantic engine already decides Mat(R) equality completely (F&S Thm 5.60; via Baez-Erbele 2015 for fields, Wadsley–Woods arXiv:1505.00048 for commutative rigs, cf. BE15 §6). The remaining question — whether to *also* make the syntactic CC engine complete — was a decision point tracked as **#15**, with two branches (kept here as the record of what was decided between):

- **(A)** Knuth-Bendix completion of the 18 Thm 5.60 equations modulo SMC coherence (would close the `cc_completeness_tracking_*` tests under CC and retire `smc_refine`), or
- **(B)** declare `MatrixNFFunctor` terminal for Mat(R) and leave the syntactic engine as-is.

**Decision (resolved): (B) functorial-terminal.** `Presentation::eq_mod_functorial` with `MatrixNFFunctor` is the terminal, complete decision procedure for Mat(R) (F&S Thm 5.53 / Thm 5.60; proof via Baez-Erbele 2015 for fields, Wadsley–Woods arXiv:1505.00048 for commutative rigs, cf. BE15 §6). The syntactic CC engine is **incomplete by design**: it stays the default `eq_mod` backend for its role (overlapping-user-equation congruence), but it is not required to be complete for Mat(R). Knuth-Bendix completion is demoted to a time-boxed feasibility spike (**#57**), relevant only for a future non-Mat(R) presentation that lacks a semantic functor. The `cc_completeness_tracking_*` depth-2 tests were re-baselined as regression trackers at the post-#189 collision counts (BoolRig 748, UnitInterval 1114, Tropical 1594, F64 1590 — all pinned exactly two-sided; prior pins 952/1397/1974/1969 post-#57-a1, 980/1433/2018/2013 post-#174, 979/1432/2017/2012 post-#55-PR2, 972/1400/1930/1925 post-#55-PR1, 1142/1634/2234/2229 post-E_18; F64 was float-nondeterministic (jitter band `2468..=2488`) until #58 normalized signed zero in the rig `Hash` impls). The #174 re-pin was **+1 on every rig** — one new collision pair, none lost, attributable to the free-site retirement rather than to the column pass. The #55-PR2 and #174 re-pins were both a **rise** and deliberate: at the time the collision count conflated NF canonicality with bounded-depth E_18 equational reach (congruence bridges co-adapted to the previous exact NFs), so the pins served as explain-the-delta trackers only — canonicality itself is decided by the `smc_canonicality_probes` module (SMC-equal pairs asserted NF-equal directly), the gate of record since #55 PR2.

The **#57-a1 re-pin is a drop, and it narrows the conflation without removing it.** `eq_mod` now settles the SMC layer by *content* equality, which decides SMC-equality exactly (Lemma 4.1, `SMC-NF-RECONCILIATION.md` §4.2), so every same-matrix pair that is SMC-equal is merged before congruence closure is consulted. That removes the **short-circuit** conflation: no residual is attributable to NF incompleteness *at the SMC layer*. It does not make the counts NF-independent — `nf` remains the canonicalizer inside `kb::CongruenceClosure`'s `smc_refine` fixpoint, so an NF change can still move these pins through the user-equation layer. #173's "conflation" note is **partially addressed**, not discharged.
...
### catgraph core  — all modules anchored to [FS19]
...
### catgraph-applied  — mixed provenance
...
| `mat_f64.rs` (feature `f64-rig`) | — | §5.3 Def 5.50 bridge | nalgebra | `mat_to_nalgebra`/`mat_from_nalgebra` + det + inverse + LU solve + SVD rank + 1-norm/Frobenius norm for F64Rig. |
