...
| Thm 6.55: spider theorem (connected diagrams = spiders) | 🔗 ✅ | catgraph::frobenius + `tests/spider_theorem.rs` | Explicit tests in catgraph core — `tests/spider_theorem.rs` (8 tests), in two halves. **§1, five term-level tests** (#110) assert *structural* equality against `special_frobenius_morphism(m, n, z)` at (2,2), (3,1), (1,3), (0,0), (2,1) — these pin the **builder's own shape**, not the theorem: `FrobeniusMorphism`'s `Eq` is syntactic up to `two_layer_simplify`, so a connected diagram the builder did not itself build is structurally ≠ the spider (six such measured, #288). **§2 asserts the theorem itself** (#288), over a finite generated corpus rather than in general: 617 terms built from η/ε/μ/δ/σ/id alone are checked at the *semantics*, via `frobenius_to_cospan` (Prop 3.8) + `Cospan::canonical_form` — 288 connected terms (both labels × (m,n) in 0..=4 × 0..=4 minus (0,0) × four decoration variants, plus a scripted wide-waist family and random walks; depth to 11; 113 carrying a σ) each land on one apex vertex covering all of 0..m / 0..n, equal to the spider's own canonical form. The corpus's structural spread is disclosed in the test's own docstring and is not uniform: a diagram of *waist* ≤ 1 (narrowest running codomain) already factors as the spider does, and 271 of the original 272 connected terms did, so `wide_waist_family` was added to put 16 waist-≥-2 shapes in deliberately (17 of 288 measured, floor asserted). Connectivity is decided by a disjoint-set over the *recipe*, never read off the oracle, and the verdict is used both ways: 220 disconnected recipes must denote exactly their own component count. `m = n = 0`, recipes that close a component, and recipes with no component at all (the empty term, 47 of them, on which the count assertion would read `0 == 0`) are excluded by design |
...
1. **Thm 6.55 (spider theorem)** — ✅ **CLOSED in catgraph core**, at the semantics rather than at the term level. `tests/spider_theorem.rs` interprets a generated corpus of connected Frobenius diagrams through `frobenius_to_cospan` (Prop 3.8) and asserts that each canonicalises to a single apex vertex carrying every domain and codomain index — the spider's own canonical form, which `special_frobenius_morphism(m, n, z)` is then compared *to* (#288). It is a wide finite sample at one instantiation (`char`/`String`), not a proof, and the shape of the sample — including the waist restriction that dominates it, and the scripted `wide_waist_family` added to answer it — is stated in the test's own docstring. Its five older term-level tests (#110) assert *structural* equality against that builder at five arities; structural equality is not the Frobenius quotient, so those pin the builder's shape and not the theorem. Connectivity is decided by a disjoint-set over the construction and checked in both directions; `m = n = 0`, component-closing recipes (the special vs extra-special line) and component-free empty recipes are excluded by design.
