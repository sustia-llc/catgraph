...
The **#57-a1 re-pin is a drop, and the conflation is gone with it.** `eq_mod` now settles the SMC layer by *content* equality, which decides SMC-equality exactly (Lemma 4.1, `SMC-NF-RECONCILIATION.md` §4.2), so every same-matrix pair that is SMC-equal is merged before congruence closure is consulted: the canonicality component of these counts is now **exactly zero** and the residual is purely depth-2 CC incompleteness on the user equations. The direction was forced rather than observed — the content relation *contains* the NF relation it replaced (`nf` preserves content, §4.3 Lemma 4.2), and a containing relation can only merge `eq_mod`-classes inside a matrix bucket, never split them, so the count could only fall. That containment is itself pinned as a test over 2000 unrelated pairs (`tests/content_equality_corpus.rs`). Note the metric's blind spot, unchanged: merges happen only *within* a matrix bucket, so these counts cannot witness a false equality across buckets — soundness rests on that file's negative controls and on `thm_5_60_soundness_*`.
