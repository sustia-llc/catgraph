...
- §5.4 Thm 5.60 is closed via the opt-in `Presentation::eq_mod_functorial<MatrixNFFunctor<R>>` semantic engine — a complete decision procedure for `Free(Σ_SFG)/⟨E_{17}⟩ ≅ Mat(R)` by Baez-Erbele 2015. The default syntactic (CC) engine remains incomplete; see the §5.4 Thm 5.60 row for the Option A +44% measured improvement and the Knuth-Bendix-completion research path (tracked as #15).
...
2. ~~**Signal flow graphs and Mat(R) (§5.3–5.4)**~~ — ✅ **CLOSED.** `SignalFlowGraph<R>` (Def 5.45), `MatR<R>` (Def 5.50), and `sfg_to_mat` functor (Thm 5.53). Thm 5.60 closed via the opt-in `Presentation::eq_mod_functorial<MatrixNFFunctor<R>>` semantic engine — complete by Baez-Erbele 2015. The syntactic (CC) engine remains incomplete: CC is the default `eq_mod` backend (closing the overlapping-user-equation branch), and Option A + Layer-1 Joyal-Street NF short-circuit cut BoolRig d=2 collisions 2574 → 1433 (~44%), but reaching zero requires either Knuth-Bendix completion (tracked as #15) or the Functorial engine. The 12 `cc_completeness_tracking_*` tests (renamed from `thm_5_60_faithful_*` to reflect what they measure) remain `#[ignore]`'d as diagnostic.
...
The `MatrixNFFunctor` semantic engine already decides Mat(R) equality completely (Baez-Erbele 2015). The remaining question — whether to make the *syntactic* CC engine complete as well — is a decision point tracked as **#15**, with two branches:
