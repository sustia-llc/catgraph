...
- **Shared `adjacent_swaps` bubble-sort core extracted**
  ...
  and `prop::presentation::smc_nf`'s `decompose_braid` now lives in one
  `pub(crate)` helper (`crate::prop::adjacent_swaps`); each call site maps the
  returned swap list into its own representation (SFG braid layers vs. reversed
  `Layer<G>` values). Behavior-preserving — both call sites' existing tests
  pass unchanged.
