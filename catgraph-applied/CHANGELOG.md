...
- **`prop::presentation::rewrite` — a process cost functional and bounded
  ...
  - `RewriteRule::new(lhs: ColoredExpr<G>, rhs: ColoredExpr<G>)` compiles an
    ...
    non-parallel sides, an arity-ill-formed side (including the
    [#196](https://github.com/sustia-llc/catgraph/issues/196) overflow screen),
    an edge-free left-hand side, and a **non-mono left interface** (a repeated
    boundary node, where the pushout complement stops being unique).
  - `optimize(start, rules, fuel, per_gen) -> RewriteOutcome<G>` searches
    best-first over states keyed by `canonical_key` — the sanctioned dedup use —
    applying convex matches (BGKSZ [arXiv:1602.06771] Def 3.10 / 5.4–5.5) and
    spending at most `fuel` rewrite applications. `RewriteOutcome` reports the
    best expression (read back via `display::expr_of_content` and re-checked
    through `ColoredExpr::new` — the re-check *is* the output validation), the
    initial and best cost, a replayable trace, whether fuel was exhausted, and
    how many states were seen. `replay` re-derives the state a trace describes,
    re-validating each step as a convex match.
  - **Claims discipline.** Per-step soundness is anchored to BGKSZ **Thm 5.6**
    ...
    it strictly refines matches, so soundness is inherited. Spiders participate
    as opaque generators only: the Frobenius unrestricted-DPO substrate
...
- **Internal:** `prop::presentation::content` gains a `pub(super)`
  ...
  BGKSZ Thm 3.12 image characterization (monogamy, the boundary biconditional,
  acyclicity) by *checking* it. Not public API; `Content`'s fields and public
