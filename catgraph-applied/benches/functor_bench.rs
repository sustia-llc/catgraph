...
//! identify. The witness count (972 for `size_bound=2` on `BoolRig`,
//! post-#55-PR1) stays nonzero **by design**: the terminal Mat(R) decision path
...
//!   972 CC-incompleteness witnesses on `BoolRig` (post-#55-PR1) — the
...
//!   `BoolRig` produces 972 witnesses (post-#55-PR1); `F64Rig` produces a
//!   larger count (1925 exact post-#55-PR1 — the tracker baseline measured on the
...
// 972 CC-incompleteness witnesses (measured empirically, post-#55-PR1; see the
...
fn bench_cc_incompleteness_count_bool(c: &mut Criterion) {
    ...
    // d=2: the canonical signal (972 witnesses, post-#55-PR1).
