...
//! identify. The witness count (979 for `size_bound=2` on `BoolRig`,
//! post-#55-PR2) stays nonzero **by design**: the terminal Mat(R) decision path
...
//!   979 CC-incompleteness witnesses on `BoolRig` (post-#55-PR2) — the
...
//!   `BoolRig` produces 979 witnesses (post-#55-PR2); `F64Rig` produces a
//!   larger count (2012 exact post-#55-PR2 — the tracker baseline measured on the
...
// 979 CC-incompleteness witnesses (measured empirically, post-#55-PR2; see the
...
fn bench_cc_incompleteness_count_bool(c: &mut Criterion) {
    ...
    // d=2: the canonical signal (979 witnesses, post-#55-PR2).
