...
**Status:** workspace version `0.20.0`. Released as workspace tags `v0.1.0`
...
the checked `mobius_chains` ζ-count conversions), `v0.6.0` (2026-08-02: the
opt-in `f64-fast` factorization route with `cond₁(ζ)` reporting, and exact
zero-diversity proofs on the coalition evaluator), and `v0.7.0` (2026-08-02:
the `coalition_typed` typed-magnitude valuation surface — role shares,
ρ-modulation + the role-grid factorization certificate, and the θ-collapse of
channel-valued couplings, #211), `v0.9.0` (2026-08-04: the rig identities
`Zero`/`One` become catgraph's own in `catgraph-applied`, re-exported here —
`deep_causality_num` retired, #219), `v0.10.0` (2026-08-09: browser-wasm
...
core/applied braiding split #258 + dl carriers #200), and `v0.16.0`
...
quotient #351), none of which carried a magnitude-facing change; and the
current `v0.17.0` (2026-09-03: `EvalPath` is `#[non_exhaustive]` and gains
`SlowNearSingular`, #307).
...
| `CoalitionEvaluator::value_with_report(candidate)` / `value_with_report_scratch(candidate, &mut EvalScratch)` → `JoinReport` | BV 2025 §3.5 Eq (7) | Additive reporting surface — same value **bit-identical** to `value_with`, plus `path()` / `schur_complement()` / `zero_proof()`; `value_with(_scratch)` untouched and pays nothing (#153) |
| `JoinReport` + `ZeroDiversityProof` + `EvalPath` | numerical | Exact zero-diversity certificates — `SkeletalMerge` (merge **∧ ¬**interior; route `EvalPath::MergeOnly`, the cached `base_value()` returned bitwise with no bordered ζ′), `Incoming`/`OutgoingProfileDuplicate` (fast-path only); a proof ⇒ the **real** increment is exactly `0` (returned value may differ by ~1e-14 roundoff — branch on the proof, never on `increment() == 0.0`), `None` ⇒ *not proven*, never "nonzero" (#153, #208) |
...
| `CoalitionEvaluator::role_shares(&[RoleId])` → `RoleShares` | Leinster 2013 §1.1 Lemma 1.1.4 (**extension**) | **T1** — decomposes `Mag = Σ w` across a **member-local** role partition; numbers unchanged, types enter reporting only (#211) |
| `RoleShares` + `MixedClass` + `RoleId` | — (**extension**) | `share(role)` (`Some(0.0)` ≠ `None`) / `is_exact()` / `mixed_classes()` / `n_roles()`; a role-mixed skeletal class is **flagged, never split** (proof-or-flagged, #153 style) — "exact" is an *attribution* claim; **when `is_exact()`**, `Σ share ≈ base_value()` up to float re-association, **when not**, the sum is short by exactly the mixed classes' weight (structural, not rounding) (#211) |
| `RoleModulation::new(rho)` + `modulate(couplings, roles, &rho)` → `ModulatedCouplings` | — (**extension**) | **T2** — validated square role table, entries in `{0} ∪ [UNIT_INTERVAL_FLOOR, 1]` (**asymmetric ρ allowed**; for downstream `magnitude_f64` consumers this limits `f64-fast` engagement, cf. #210 — the coalition path never calls that route) applied as `π′ = ρ(r_i, r_j)·π` (`d′ = d_ρ + d`); `roles` is **agent-indexed**; output feeds every existing entry point, which rejects a modulated product that fell into `(0, UNIT_INTERVAL_FLOOR)` (#211) |
| `role_grid(&role_space, &fiber)` → `RoleGrid` | Leinster 2013 Prop 1.4.3 / Prop 2.3.6 (**extension**) | **T2** — product coalition, agents `(r, x)` at `r·n + x`, `π((r,x),(r′,x′)) = ρ(r,r′)·σ(x,x′)`; **both factor diagonals must be exactly `1.0`** (the engine forces a unit diagonal, and `(ρ⊗σ)` with forced diag `= (ρ)⊗(σ)` only then) (#211) |
| `RoleGrid::proof(t)` → `RoleFibrationProof` | Leinster 2013 Prop 2.3.6 + Leinster 2008 Prop 2.8 (**extension**) | **T2** — `role_magnitude()` / `fiber_magnitude()` / `expected_magnitude()` / `t()`, each factor via the existing `coalition_magnitude_from_couplings`. **Construction-carried and mathematical** — no float detection on arbitrary inputs; the two evaluation routes re-associate differently, so compare within a **relative tolerance** (#211) |
| `ChannelCouplings::{new, set, n_channels, collapse}` | Leinster 2013 §1.3 (**extension**) | **T3** — per-pair vectors `v ∈ ({0} ∪ [UNIT_INTERVAL_FLOOR, 1])^C` collapsed by the declared monoid hom `\|v\|_θ = Π v_c^{θ_c}` (`d = Σ θ_c d_c`); a collapsed coupling below the floor is rejected by the entry points. θ finite and `≥ 0`; **`Σθ = 1` is a convention, not enforced** (float knife-edge). `θ = e_c` recovers channel `c` exactly; `0^0 = 1`, so `θ = 0` collapses everything to `1.0` (degenerate, caller's call). No canonical θ exists (#211) |
...
- ✅ **`v0.2.0`** (2026-07-02, #31/PR #32): incremental coalition magnitude for
  ...
  contract.
- ✅ **`v0.4.0`** (2026-07-25, #33/#35/#53): integer-SNF numerics
  (`hadamard_bound_matr`/`hadamard_bound_integer`, polynomial-DP chain rebalance,
  const prime table, `crt_lift` split into `snf::crt` + `snf::integer`);
