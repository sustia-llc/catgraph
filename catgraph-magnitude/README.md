...
| `RoleShares` + `MixedClass` + `RoleId` | — (**extension**) | `share(role)` (`Some(0.0)` ≠ `None`) / `is_exact()` / `mixed_classes()` / `n_roles()`; a role-mixed skeletal class is **flagged, never split** (proof-or-flagged, #153 style) — "exact" is an *attribution* claim, `Σ share ≈ base_value()` only up to float re-association (#211) |
| `RoleModulation::new(rho)` + `modulate(couplings, roles, &rho)` → `ModulatedCouplings` | — (**extension**) | **T2** — validated square `[0,1]` role table (**asymmetric ρ allowed**; reduces `f64-fast` engagement, cf. #210) applied as `π′ = ρ(r_i, r_j)·π` (`d′ = d_ρ + d`); `roles` is **agent-indexed**; output feeds every existing entry point unchanged (#211) |
