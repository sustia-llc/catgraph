...
**Status:** workspace version `0.19.0`. Released as workspace tags `v0.1.0`
...
| `RoleModulation::new(rho)` + `modulate(couplings, roles, &rho)` → `ModulatedCouplings` | — (**extension**) | **T2** — validated square `[0,1]` role table (**asymmetric ρ allowed**; for downstream `magnitude_f64` consumers this limits `f64-fast` engagement, cf. #210 — the coalition path never calls that route) applied as `π′ = ρ(r_i, r_j)·π` (`d′ = d_ρ + d`); `roles` is **agent-indexed**; output feeds every existing entry point unchanged (#211) |
...
| `ChannelCouplings::{new, set, n_channels, collapse}` | Leinster 2013 §1.3 (**extension**) | **T3** — per-pair vectors `v ∈ [0,1]^C` collapsed by the declared monoid hom `\|v\|_θ = Π v_c^{θ_c}` (`d = Σ θ_c d_c`). θ finite and `≥ 0`; **`Σθ = 1` is a convention, not enforced** (float knife-edge). `θ = e_c` recovers channel `c` exactly; `0^0 = 1`, so `θ = 0` collapses everything to `1.0` (degenerate, caller's call). No canonical θ exists (#211) |
