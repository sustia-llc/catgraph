...
**Status:** workspace version `0.17.0`. Released as workspace tags `v0.1.0`
...
| `JoinReport` + `ZeroDiversityProof` + `EvalPath` | numerical | Exact zero-diversity certificates — `SkeletalMerge` (merge **∧ ¬**interior), `Incoming`/`OutgoingProfileDuplicate` (fast-path only); a proof ⇒ the **real** increment is exactly `0` (returned value may differ by ~1e-14 roundoff — branch on the proof, never on `increment() == 0.0`), `None` ⇒ *not proven*, never "nonzero" (#153) |
