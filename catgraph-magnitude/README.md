...
| `CoalitionEvaluator::value_with_report(_scratch)(candidate)` → `JoinReport` | BV 2025 §3.5 Eq (7) | Additive reporting surface — same value **bit-identical** to `value_with`, plus `path()` / `schur_complement()` / `zero_proof()`; `value_with(_scratch)` untouched and pays nothing (#153) |
