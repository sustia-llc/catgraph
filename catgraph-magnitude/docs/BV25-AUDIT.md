...
|---|---|---|---|
| §1.1 Def 1.1.1 + Lemma 1.1.2 + Lemma 1.1.4: weighting / coweighting | ✅ | `magnitude::weighting`, `magnitude::coweighting` | v0.2.0. Both solve `ζ · w = u_I` (resp. `v · ζ = u_J^T`) by Gaussian-Jordan elimination on the augmented system. By Lemma 1.1.2, `Σⱼ w(j) = Σᵢ v(i) = magnitude`. By Lemma 1.1.4, on invertible ζ, `w(j) = Σᵢ μ(j, i)` (row-sum of `μ = ζ⁻¹`). Verified in `tests/weighting_coweighting.rs` (including Lemma 1.1.2 + Lemma 1.1.4 numerical residuals). Bound: `Q: Ring + Div + From<f64>` matches `mobius_function`. |
